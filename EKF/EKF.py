...
class UserCode:
    ...
    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
        ...
        jacob[0,0]=-cos(x[2])
        jacob[0,1]=-sin(x[2])
        jacob[0,2]=-(marker_position_world[0]-x[0])*sin(x[2])+(marker_position_world[1]-x[1])*cos(x[2])
        jacob[1,0]=sin(x[2])
        jacob[1,1]=-cos(x[2])
        jacob[1,2]=-(marker_position_world[0]-x[0])*cos(x[2])-(marker_position_world[1]-x[1])*sin(x[2])
        jacob[2,0]=0
        jacob[2,1]=0
        jacob[2,2]=-1
        return jacob
